...
var S3CMD_CONFIG = process.env.HOME + '/.s3cmd';
...
try {
...
} catch (err) { // Backup is the s3cmd file.
    if (err.code !== 'ENOENT') throw err;
    var iniparser = require('iniparser');

    try {
	var config = iniparser.parseSync(S3CMD_CONFIG);
	if (!config.default || !config.default.access_key || !config.default.secret_key) {
	    console.log(S3CMD_CONFIG + " doesn't have a [default] section with access_key and secret_key");
	    process.exit(-1);
	}
	AWS.config.update({
	...
	if (err.code !== 'ENOENT') throw err;
	console.log("I need a config file: " + CONFIG_FILE + " or " + S3CMD_CONFIG);
	process.exit(-1);
    }
}
...
var reportOnQueue = function(queue, cb) {
    ...
    if (onlywatch.indexOf(queueName) >= 0 || opts.all) {
